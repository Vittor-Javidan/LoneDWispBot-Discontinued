...
export default function equipment_BodyArmorInventory(data) {
	...
	if (words[0] === '!cs') {
		...
		const allEquipmentString = playerInstance.getInventoryEquipmentsString(CS_ENUM.KEYS.CS_ENTITY_EQUIPMENT.BODY_ARMOR)
        sendMessage(
    ...
    switch (true) {
        ...
        case (itemCode <= playerInstance.getInvetoryEquipments(CS_ENUM.KEYS.CS_ENTITY_EQUIPMENT.BODY_ARMOR).length):

            playerInstance.setEquippedEquipment(itemCode, CS_ENUM.KEYS.CS_ENTITY_EQUIPMENT.BODY_ARMOR)
            playerInstance.setSecondaryState(CS_ENUM.STATES.RESTING.SECONDARY.EQUIPMENT_BODY_ARMOR)
            sendMessage(
