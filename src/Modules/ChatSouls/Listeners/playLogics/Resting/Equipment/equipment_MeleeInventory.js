...
export default function equipment_MeleeInventory(data) {
	...
	if (words[0] === '!cs') {
        ...
        const allEquipmentString = playerInstance.getInventoryEquipmentsString(CS_ENUM.KEYS.CS_ENTITY_EQUIPMENT.MELEE_WEAPON)
        sendMessage(
    ...
    switch (true) {
        ...
        case (itemCode <= playerInstance.getInvetoryEquipments(CS_ENUM.KEYS.CS_ENTITY_EQUIPMENT.MELEE_WEAPON).length):

            playerInstance.setEquippedEquipment(itemCode, CS_ENUM.KEYS.CS_ENTITY_EQUIPMENT.MELEE_WEAPON)
            playerInstance.setSecondaryState(CS_ENUM.STATES.RESTING.SECONDARY.EQUIPMENT_MELEE)
            sendMessage(
