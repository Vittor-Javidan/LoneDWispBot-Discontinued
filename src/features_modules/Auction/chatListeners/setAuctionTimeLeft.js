...
export default function setAuctionTimeLeft(message) {
	...
	//Is instance null?
	if(!instance){
		sendChatMessage(`O leilão de código "${itemCode}" não existe`)
		return
	}
	...
	sendChatMessage(`Tempo restante do Leilão[${instance.getItemName()}] foi modificado para ${minutes} minutos`)
}

function isInvalidMessageLenght(words){
	
	if(words.length !== 4){
		sendChatMessage(`commando inválido.`)
		return true
	}
	return false
}
